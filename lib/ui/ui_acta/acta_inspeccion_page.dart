...
class _RegisterPageState extends State<RegisterPage> {
  ...
  void initState() {
    ...
    espejosController = TextEditingController(
        text: Provider.of<ActaState>(context, listen: false).MapOfValue['ACTA']['TEXT_CAMP']["Espejos"]);
    focosDelanterosController = TextEditingController(
        text: Provider.of<ActaState>(context, listen: false).MapOfValue['ACTA']['TEXT_CAMP']["Focos faeneros delanteros"]);
    focosTraserosController = TextEditingController(
        text: Provider.of<ActaState>(context, listen: false).MapOfValue['ACTA']['TEXT_CAMP']["Focos faeneros traseros"]);

    llaveController = TextEditingController(
        text: Provider.of<ActaState>(context, listen: false).MapOfValue['ACTA']['TEXT_CAMP']["LLave de contacto"]);
    intermitentesDelanterosController = TextEditingController(
        text:Provider.of<ActaState>(context, listen: false).MapOfValue['ACTA']['TEXT_CAMP']["Intermitentes delanteros"]);
    intermitentesTraserosController = TextEditingController(
        text:Provider.of<ActaState>(context, listen: false).MapOfValue['ACTA']['TEXT_CAMP']["Intermitentes traseros"]);
    ruedasController = TextEditingController(
        text: Provider.of<ActaState>(context, listen: false).MapOfValue['ACTA']['TEXT_CAMP']["Ruedas"]);
  ...
  Widget build(BuildContext context) {
    ...
    allValue  = Provider.of<ActaState>(context).MapOfValue['ACTA']['SELECT_CAMP'];
                                          ...
                                          child: SizedBox(
                                            child: Row(
                                              children: [
                                                ...
                                                RenderSpecialFields(tipo: widget.tipo, keyMap: key,),
                                                Provider.of<ActaState>(
                                                    context,
                                                    listen:
                                                    false)
                                                    .MapOfValue['ACTA']['TEXT_CAMP'][
                                                key] !=
                                                    null
                                                    ? Padding(
                                                  padding:
                                                ...
                                                CheckBoxWidget(fontSizeTextHeader: fontSizeTextHeader, allValue: allValue, TextHeader: 'Bueno', PositionArray: 0,
                                                  index: index, position: position, keyR: key,
                                                ),
                                                CheckBoxWidget(fontSizeTextHeader: fontSizeTextHeader, allValue: allValue, TextHeader: 'Regular', PositionArray: 1,
                                                  index: index, position: position, keyR: key,
                                                ),
                                                CheckBoxWidget(fontSizeTextHeader: fontSizeTextHeader, allValue: allValue, TextHeader: 'Malo', PositionArray: 2,
                                                  index: index, position: position, keyR: key,
                                                ),
                                              ],
                                            ),
...
class CheckBoxWidget extends StatelessWidget {
  const CheckBoxWidget({required this.fontSizeTextHeader,required this.allValue,
  required this.position,required this.keyR, required this.TextHeader,required this.PositionArray, required this.index}) : super();
            ...
            child: Checkbox(
                ...
                onChanged: (bool?value) {
                  if (allValue[position][keyR]!.contains(true)) {
                    int indexl = allValue[position][keyR]!.indexWhere((element) => element == true);
...
class RenderSpecialFields extends StatefulWidget {
  ...
  const RenderSpecialFields({Key? key,required this.keyMap, required this.tipo}) : super(key: key);
  final keyMap;
  final bool tipo;
  @override
...
class _RenderSpecialFieldsState extends State<RenderSpecialFields> {
  ...
  void didChangeDependencies() {
    ...
    var provider= Provider.of<ActaState>(
        context,
        listen:
        false);

    data = Provider.of<ActaState>(
        context,
        listen:
        false).MapOfValue['ACTA']['OPTION_SELECT'];
    ...
    if(widget.tipo){
      ...
      mapSetFunct={
        'Carro y su respaldo de carga':provider.setFieldSpecialRespaldoElectrico,
      ...
      bateriaController = TextEditingController(
          text: Provider.of<ActaState>(context, listen: false).MapOfValue['ACTA']['OPTION_SELECT']['Bateria'].select);
      serieController =TextEditingController(
          text:  Provider.of<ActaState>(context, listen: false).MapOfValue['ACTA']['OPTION_SELECT']['Serie cargador'].select);
      listController = {
          ...
          Row(
            children: [
              ...
              DropdownButton<String>(
                ...
                items: data[widget.keyMap].options
